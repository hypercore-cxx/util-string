...
int main () {
  ...
  {
    auto r = Hyper::Util::String::search(f, "\\w+");
    ASSERT("search results == 5", r.size() == 5);
  }

  {
    auto r = Hyper::Util::String::split(f, "/");
    ASSERT("parts after split == 4", r.size() == 4);
  }

  {
    auto r = Hyper::Util::String::replace("Hello, World!", "o", "x");
...

  {
    ...
    auto r = Hyper::Util::String::replace(s, "l", [&](string const& m, int index) {
  ...
  {
    auto r = Hyper::Util::String::trim("!Hello, World!!", "!");
    ASSERT("trim", r == "Hello, World");
  }

  {
    auto r = Hyper::Util::String::rtrim("!Hello, World!!", "!");
    ASSERT("rtrim", r == "!Hello, World");
  }

  {
    auto r = Hyper::Util::String::ltrim("!Hello, World!!", "!");
    ASSERT("ltrim", r == "Hello, World!!");
  }

  {
    auto r = Hyper::Util::String::match(f, "/(\\w+)/(.*)");
    ASSERT("parts after match == 2", r.size() == 3);
  }

  {
    auto r = Hyper::Util::String::match("/foo/bar/", "/(\\w+)/(\\w+)/");
    ASSERT("match (passing)", r[1] == "foo" && r[2] == "bar");
  }

  {
    auto r = Hyper::Util::String::match("crap", "/(\\w+)/(\\w+)/");
    ASSERT("match (failing)", r.empty());
  }

  {
    auto r = Hyper::Util::String::test("f", "[a-zA-Z]");
    ASSERT("test (passing)", r == true);
  }

  {
    auto r = Hyper::Util::String::test("1", "[a-zA-Z]");
    ASSERT("test (failing)", r == false);
  }

  {
    auto r = Hyper::Util::String::test(f, "^/(\\w+)/(\\w+)/(\\w+)-path.js$");
